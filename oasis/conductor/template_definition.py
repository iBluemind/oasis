...
class TemplateDefinition(object):
    ...
    def load_entry_points():
        for entry_point in iter_entry_points('oasis.template_definitions'):
    ...
    def get_template_definitions(cls):
        ...
        if not cls.definitions:
            ...
            for entry_point, def_class in cls.load_entry_points():
                for function_type in def_class.provides:
                    function_type_tuple = (function_type['server_type'],
                                      function_type['os'],
                                      function_type['coe'])
                    providers = cls.definitions.setdefault(function_type_tuple,
                                                           dict())
    ...
    def get_template_definition(cls, server_type, os, coe):
        ...
        if bay_type not in definition_map:
            raise exception.FunctionTypeNotSupported(
...
class BaseTemplateDefinition(TemplateDefinition):
    def __init__(self):
        ...
        self.add_parameter('flavor')
        self.add_parameter('image')
        self.add_parameter('private_network')
        self.add_parameter('public_network')
        self.add_parameter('subnet')
        self.add_parameter('key_name')
